...
public class GameServer extends Thread {
  ...
  public GameServer(AbstractGame game) {
    this.game = game;
    try {
  ...
  private void parsePacket(byte[] data, InetAddress address, int port) {
    ...
    switch (type) {
      ...
      case LOGIN:
        ...
        PlayerMP player = new PlayerMP(100, 100, ((Packet00Login) packet).getUsername(), address, port, game);
  ...
  public void addConnection(PlayerMP player, Packet00Login packet) {
    ...
    for (PlayerMP p : this.connectedPlayers) {
      if (player.getUsername().equalsIgnoreCase(p.getUsername())) {
        ...
        // exists
        packet = new Packet00Login(p.getUsername(), p.posX, p.posY);
  ...
  private void handleMove(Packet02Move packet) {
    if (getPlayerMP(packet.getUsername()) != null) {
      ...
      player.posX = packet.getX();
      player.posY = packet.getY();
