...
public abstract class World implements LevelUtils.TileWorld {
  ...
  public Tile getTileAt(int tileX, int tileY) {
    return staticMapContent.getTileAt(tileX, tileY);
  }
  ...
  public boolean canGapJumpAtPosition(int x, int y, int moveDir) {
    ...
    for (var i = 0; i < 4; i++) {
      if (staticMapContent.getTileAt(tx + (moveDir * i), ty - 1).isSolid()) {
        return false;
      }

      if (staticMapContent.getTileAt(tx + (moveDir * i), ty).isSolid()) {
        return true;
      }
    }
  ...
  public boolean canJumpUp(int x, int y) {
    ...
    if (staticMapContent.getTileAt(tx, ty - 1).isSolid()) {
      return false;
    }

    if (staticMapContent.getTileAt(tx, ty - 2).isSolid()) {
      return false;
    }

    if (staticMapContent.getTileAt(tx, ty - 3).isSolid()) {
      return false;
    }

    if (staticMapContent.getTileAt(tx, ty - 4).isSolid()) {
      return false;
    }

    if (!staticMapContent.getTileAt(tx, ty - 5).isSolid()) {
      return false;
    }

    return !staticMapContent.getTileAt(tx, ty - 6).isSolid();
  }
